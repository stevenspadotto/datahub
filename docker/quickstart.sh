...
if podman volume ls | grep -c -q datahub_neo4jdata
then
  echo "Datahub Neo4j volume found, starting with neo4j as graph service"
  cd $DIR && podman-compose pull && podman-compose -p datahub up
else
  echo "No Datahub Neo4j volume found, starting with elasticsearch as graph service"
  cd $DIR && \
  DOCKER_DEFAULT_PLATFORM="$(uname -m)" podman-compose -p datahub \
